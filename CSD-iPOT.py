...
def CSD(args):
    ...
    cdf_valid_all = util.get_cdf_matrix(pred_params_valid, tgt_valid, args).cpu()
    cdf_test_all = util.get_cdf_matrix(pred_params_test, tgt_test, args).cpu()
    ...
    if args.model_dist != 'cox':
        ...
        cdf_valid_all = cdf_valid_all.cpu()[:, order_valid][order_valid, :]
        cdf_test_all = cdf_test_all.cpu()[:, order_test][order_test, :]
        cdf_valid = cdf_valid.cpu()[order_valid]
    ...
    con_scores_event = np.repeat(surv_con[is_dead_valid.cpu() == 1].cpu().numpy(), R)
    con_scores_cen = grid * surv_con[is_dead_valid.cpu() == 0].cpu().numpy().reshape(1, -1)
    ...
    _, KS = util.get_p_value(args=args, cdf=torch.tensor(1-surv_test), is_dead=is_dead_test.cpu())
