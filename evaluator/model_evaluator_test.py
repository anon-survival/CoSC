...
class ModelEvaluator_test(object):
    ...
    def __init__(self, args, data_loaders, epochs_per_eval=1):
        ...
        if self.model_dist in ['cat', 'mtlr', 'psr']:
    ...
    def _eval_phase(self, model, data_loader, phase, device):
        ...
        for src, tgt in data_loader:
            ...
            all_cdf.append(cdf)
            if self.model_dist == 'cox':
                all_src.append(src[order])
            else:
                all_src.append(src)

            if self.args.model_dist == 'psr':
                loss = loss_fn.loss(pred_params, tgt)
        ...
        all_cdf = torch.clamp(all_cdf, 0, 1)
        ...
        if phase == 'test':
            train_tte = torch.load("./train_tte.pt")
            train_event = torch.load("./train_event.pt")
            import os
            ...
            cdf_km, tte_km, is_dead_km, src_km, order_km = util.cdf_all_points(args=self.args)
            ...
            IBS = util.integrated_brier_score(train_tte=train_tte, train_event=train_event, 
                                                test_tte=tte_km, test_event=is_dead_km, 
                                                cdf_test=cdf_km, time=tte_km)
            cen_log_simple = util.cen_log_simple(tte=tte_km, is_dead=is_dead_km, cdf_matrix=cdf_km)
