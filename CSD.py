...
def CSD(args, *, model=None, train_batch=None, valid_batch=None, test_batch=None,
    ...
    model.eval()
    # Injected simulation models already carry their full-training baseline.
    # This never fits from the calibration/prediction cohort below.
    util.prepare_cox_baseline(args, model)

    train_loader = ([train_batch] if train_batch is not None else
                    util.get_train_loader(args, during_training=False))
    if valid_batch is None or test_batch is None:
        valid_loader, test_loader = util.get_eval_loaders(during_training=False, args=args)
    else:
        valid_loader, test_loader = [valid_batch], [test_batch]
    ...
    cdf_valid = util.get_cdf_matrix(pred_params_valid, tgt_valid, args).cpu()
    cdf_test = util.get_cdf_matrix(pred_params_test, tgt_test, args).cpu()

    if args.model_dist != 'cox':
        order_valid = order_valid.cpu()
        order_test = order_test.cpu()
        cdf_valid = cdf_valid.cpu()[:, order_valid][order_valid, :]
        cdf_test = cdf_test.cpu()[:, order_test][order_test, :]
    ...
    n_quantiles = getattr(args, "baseline_n_quantiles", 9)
    ...
    R = getattr(args, "baseline_completion_repeats", 1000)
    _, t_r, _ = compute_decensor_times([tte_valid.cpu(), is_dead_valid.cpu(), src_valid.cpu()],
                                      [tte_train, is_dead_train, src_train], n_sample=R)
    conformal_score = np.repeat(quantile_prediction_valid, R, axis=0) - np.array(t_r).reshape(-1, 1)
    ...
    # print("Quantile:", quantile_prediction_test)
    csd_quantile, quantile_prediction_test = make_mono_quantiles(
        csd_quantile, quantile_prediction_test, method='bootstrap',
        num_bs=getattr(args, 'baseline_rearrangement_samples', 1000000))
    ...
    if return_details:
        return dict(quantile_levels=csd_quantile, quantile_predictions=quantile_prediction_test,
                    adjustment=errors, test_order=order_test.cpu().numpy(),
                    test_time=tte_test.cpu().numpy(), test_event=is_dead_test.cpu().numpy())
    end_time = time.time()
    ...
    D_cal = util.d_calibration(points=torch.tensor(1-surv_test), is_dead=is_dead_test.cpu(), args=args, phase=args.phase, nbins=args.num_xcal_bins)
    ...
    cal_ws = util.wsc_xcal(X=src_test, event_indicators=is_dead_test, predict_probs=torch.tensor(surv_test))
    ...
    # sheet.cell(row=last_row+1, column=2, value=(f'CSD_{args.dataset}_{args.model_dist}'))
    ...
    # sheet.cell(row=last_row+1, column=2, value=KS.item())
    # sheet.cell(row=last_row+1, column=3, value=KM_cal.item())
    # sheet.cell(row=last_row+1, column=4, value=IBS.item())
    # sheet.cell(row=last_row+1, column=5, value=PSR.item())
    # sheet.cell(row=last_row+1, column=6, value=cal_ws.item())
    # sheet.cell(row=last_row+1, column=7, value=(f'{args.dataset}_{args.model_dist}_CSD'))
